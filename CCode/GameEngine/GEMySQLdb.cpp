...
*/
#pragma warning( push )
#pragma warning(disable : 4013 4996)
...
static PyObject *_mysql_server_init(
	...
	PyObject *args,
	PyObject *kwargs) {
	static char*  kwlist[] = {(char*)"args", (char*)"groups", NULL};
	...
	if (cmd_args) {
		...
		cmd_argc = (int)PySequence_Size(cmd_args);
		if (cmd_argc == -1) {
			PyErr_SetString(PyExc_TypeError,
	...
	if (groups) {
		...
		groupc = (int)PySequence_Size(groups);
		if (groupc == -1) {
			PyErr_SetString(PyExc_TypeError,
...
_mysql_ResultObject_Initialize(
	...
	PyObject *kwargs)
{
	static char *kwlist[] = {(char*)"connection", (char*)"use", (char*)"converter", NULL};
	MYSQL_RES *result; 
	...
	for (i=0; i<n; i++) {
		...
		if (PySequence_Check(fun)) {
			int j, n2=(int)PySequence_Size(fun);
			PyObject *fun2=NULL;
			for (j=0; j<n2; j++) {
	...
	PyObject *kwargs)
{
	...
	static char *kwlist[] = {(char*)"host", (char*)"user", (char*)"passwd", (char*)"db", (char*)"port",
				  (char*)"unix_socket", (char*)"conv",
				  (char*)"connect_timeout", (char*)"compress",
				  (char*)"named_pipe", (char*)"init_command",
				  (char*)"read_default_file", (char*)"read_default_group",
				  (char*)"client_flag", (char*)"ssl",
				  (char*)"local_infile",
				  NULL } ;
	...
	PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "i", &flags))
		return NULL;
	Py_BEGIN_ALLOW_THREADS
	err = mysql_set_server_option(&(self->connection), enum_mysql_set_option(flags));
	Py_END_ALLOW_THREADS
	if (err) return _mysql_Exception(self);
	return PyInt_FromLong(err);
}		
	...
	PyObject *d)
{
	...
	quoted = PyObject_CallFunction(itemconv, (char*)"OO", item, d);
	Py_DECREF(itemconv);
error:
	return quoted;
}
	...
	PyObject *args)
{
	...
	if ((n = (int)PyObject_Length(o)) == -1) goto error;
...
{
	typedef PyObject *_PYFUNC(_mysql_ResultObject *, MYSQL_ROW);
	static char *kwlist[] = {(char*)"maxrows", (char*)"how", NULL };
	...
	PyObject *kwargs)
{
	...
	int r;
        static char *kwlist[] = { (char*)"user", (char*)"passwd", (char*)"db", NULL } ;
	...
	PyObject *args)
{
	...
	r = mysql_row_tell(self->result);
	return PyInt_FromLong((long)(r-self->result->data->data));
}
...
static MyMemberlist(_mysql_ConnectionObject_memberlist)[] = {
	MyMember(
		...
		RO,
		(char*)"True if connection is open"
		),
	MyMember(
		(char*)"converter",
		T_OBJECT,
	...
	MyMember(
		...
		RO,
		(char*)"Capabilites of server; consult MySQLdb.constants.CLIENT"
		),
	MyMember(
		 (char*)"port",
		 T_UINT,
		 ...
		 RO,
		 (char*)"TCP/IP port of the server connection"
		 ),
	MyMember(
		 (char*)"client_flag",
		 T_UINT,
		 ...
		 (char*)"Client flags; refer to MySQLdb.constants.CLIENT"
		 ),
	{NULL} /* Sentinel */
};
...
void init_mysql(void)
{
	...
	if (PyDict_SetItemString(dict, "version_info",
			       PyRun_String("(1,2,3,'final',0)", Py_eval_input,
				       dict, dict)))
		goto error;
	if (PyDict_SetItemString(dict, "__version__",
			       PyString_FromString("1.2.3")))
		goto error;
...
{
	if (PyObject_IsInstance(pObj, (PyObject*)(&_mysql_ConnectionObject_Type)))
	{
		return (_mysql_ConnectionObject*)pObj;
	}
	else
	{
		return NULL;
